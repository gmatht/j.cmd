...
const rt = createSh2Runtime({ fs, env: {}, shellExec, stdout: { write: () => {} }, stderr: { write: (s) => errs.push(s) }, args: [], argv0: "bash" });
const origWrite = rt.sh2.fs.writeFile.bind(rt.sh2.fs);
rt.sh2.fs.writeFile = async (p, data) => {
  try { return await origWrite(p, data); }
  catch (e) { if (String(p).includes("blocks")) console.log("blocks write FAILED:", e && e.message, "code:", e && e.code); throw e; }
};
const fn = new Function("args", "fs", "env", "stdout", "stderr", "__runCmd", "sh2", "return (async () => { " + js + " })();");
try { await Promise.race([fn([], fs, {}, { write: () => {} }, { write: (s) => errs.push(s) }, shellExec, rt.sh2), new Promise((_, rej) => setTimeout(() => rej(new Error("stop")), 30000))]); }
