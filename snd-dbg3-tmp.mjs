...
async function runGame(args, keys, opts = {}) {
  ...
  try { const st = await fs.stat("/tmp/sound-hit.sh"); const c = await fs.read("/tmp/sound-hit.sh"); process.stderr.write("DBG staged hit.sh bytes=" + String(c).length + "\n"); } catch(e) { process.stderr.write("DBG no staged hit.sh\n"); }
