...
if (sh2glslBin) {
  check("fragment shader was generated from bash (sh→GLSL backend)",
    fragSrc.includes("generated by the sh→GLSL backend") &&
    fragSrc.includes("varying vec4 vColor;") &&
    fragSrc.includes("gl_FragColor = vec4(float(out_buf[0])"),
    fragSrc.split("\n").slice(0, 4).join(" | "));
  check("fragment shader has the bash-computed effects",
    fragSrc.includes("g_scan") && fragSrc.includes("g_corrupt") && fragSrc.includes("g_edge") &&
    fragSrc.includes("putCh(g_r)"),
    fragSrc.split("\n").filter((l) => l.includes("g_scan") || l.includes("g_corrupt")).join(" | "));
} else {
  check("fragment shader fallback embedded ES 1.00", fragSrc.includes("gl_FragColor = vec4(vColor.rgb, 1.0)"));
}
