...
// fix clamps only the NEGATIVE half of w to a small positive floor in
// the vertex shader (both the hand-written vs_fb and the generated
// GLSL, injected by the game): a magnitude clamp is wrong both ways —
// 0.5 jumps MIME faces (same-row mimes are scale 0.7 → w=0.35) and
// 0.0001 lets the back half divide to ±∞ (face explodes). The camera
// sits at the cell centre, so every genuinely-in-front face has w ≥ 0.5
...
check("game injects the negative-only clamp into the generated GLSL", src.includes("if (g_w < 0.0) g_w = 0.05;"));
...
check("generated GLSL has the negative-only clamp after injection", vsGlsl.includes("if (g_w < 0.0) g_w = 0.05;"));
