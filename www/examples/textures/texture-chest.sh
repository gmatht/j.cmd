#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
# texture-chest.sh — treasure chest side: wood planks with iron struts
# and a latch/lock plate, for the block(s) facing the player.
#
#   bash texture-chest.sh > chest.ppm        # PPM P6, 16×16 (default)
#   bash texture-chest.sh --preview           # color + shade preview
#   bash texture-chest.sh --png               # chest-<seed>.png (needs convert)
#   TEX_SIZE=32 TEX_SEED=7 bash texture-chest.sh > chest32.ppm
#
...
# ─── body ───────────────────────────────────────────────────────────────────
# treasure chest side (viewed from the front):
#   • wood planks everywhere (vertical grain, light oak)
#   • a darker curved-lid band across the top quarter, with a hard seam
#   • an iron band (horizontal strut) across the middle, with rivets
#   • vertical corner struts below the band on both edges (chest legs)
#   • a gold latch/lock plate on the band, centered, with a keyhole
RB=166
GB=119
BB=63
IR=122
...
LG=172
LB=74
# derived geometry — precomputed so no $(( ... )) appears inside a test
# bracket (transpiler discipline: only plain comparisons in [ ])
BAND_TOP=$(( SIZE / 2 - 1 ))       # top row of the iron band
BAND_BOT=$(( BAND_TOP + 2 ))       # bottom row of the iron band
LID_Y=$(( SIZE / 4 ))              # first body row; lid is above
LATCH_X0=$(( SIZE / 2 - 2 ))       # latch plate left column
LATCH_X1=$(( LATCH_X0 + 4 ))       # latch plate right column + 1
LATCH_Y0=$(( BAND_TOP - 1 ))       # latch plate top row
LATCH_Y1=$(( LATCH_Y0 + 4 ))       # latch plate bottom row + 1
STRUT_R0=$(( SIZE - 3 ))           # right strut start column
STRUT_R1=$(( SIZE - 2 ))           # right strut end column (inclusive)
RIVET_X=$(( SIZE / 4 ))            # first rivet column
RIVET_X2=$(( SIZE * 3 / 4 - 1 ))   # second rivet column
KEY_X=$(( LATCH_X0 + 2 ))          # keyhole column (plate center)
KEY_Y0=$(( LATCH_Y0 + 1 ))         # keyhole top row
KEY_Y1=$(( LATCH_Y0 + 2 ))         # keyhole bottom row
...
while [ "$y" -lt "$SIZE" ]; do
  ...
  while [ "$x" -lt "$SIZE" ]; do
    # ── wood base ────────────────────────────────────────────────────
    # vertical grain: period-5 stripes with per-column phase jitter
    lat_hash $x 0 $SIZE 1
    gph=$(( lhn % 3 ))
    stripe=$(( (x + gph) % 5 ))
    coff=$(( (lhn - 128) / 5 ))
    ...
    g=$(( g + coff ))
    b=$(( b + coff ))
    # fine per-pixel grain (pure lattice hash, never horizontal smears)
    ...
    # ── lid: darker wood, then a hard seam at the body start ────────
    if [ "$y" -lt "$LID_Y" ]; then
      r=$(( r * 80 / 100 ))
      g=$(( g * 76 / 100 ))
      b=$(( b * 68 / 100 ))
    ...
    # ── iron struts ──────────────────────────────────────────────────
    # horizontal band across the middle; vertical legs on both edges
    # below the band; the band's top/bottom edges get a shadow
    ONMETAL=0
    if [ "$y" -ge "$BAND_TOP" ]; then
      if [ "$y" -le "$BAND_BOT" ]; then ONMETAL=1; fi
    fi
    if [ "$y" -gt "$BAND_BOT" ]; then
      if [ "$x" -ge 1 ]; then
        if [ "$x" -le 2 ]; then ONMETAL=1; fi
      fi
      if [ "$x" -ge "$STRUT_R0" ]; then
        if [ "$x" -le "$STRUT_R1" ]; then ONMETAL=1; fi
      fi
    fi
    if [ "$ONMETAL" -eq 1 ]; then
      # brushed iron: horizontal brushing via column phase + noise
      lat_hash $x $y $SIZE $SIZE
      met=$(( (lhn - 128) / 4 ))
      r=$(( IR + met ))
      g=$(( IG + met ))
      b=$(( IB + met ))
      # band top/bottom edges darker (the strut shadow)
      if [ "$y" -eq "$BAND_TOP" ]; then
        r=$(( r * 80 / 100 ))
        g=$(( g * 80 / 100 ))
        b=$(( b * 82 / 100 ))
      fi
      if [ "$y" -eq "$BAND_BOT" ]; then
        r=$(( r * 84 / 100 ))
        g=$(( g * 84 / 100 ))
        b=$(( b * 86 / 100 ))
      fi
      # rivets: dark dots on the band's middle row, between the latch
      # and the corners
      if [ "$y" -eq "$BAND_TOP" ]; then
        if [ "$x" -eq "$RIVET_X" ]; then
          r=$(( r * 55 / 100 ))
          g=$(( g * 57 / 100 ))
          b=$(( b * 63 / 100 ))
        fi
        if [ "$x" -eq "$RIVET_X2" ]; then
          r=$(( r * 55 / 100 ))
          g=$(( g * 57 / 100 ))
          b=$(( b * 63 / 100 ))
        fi
    ...
    # ── gold latch/lock plate (straddles the band, centered) ────────
    if [ "$x" -ge "$LATCH_X0" ]; then
      if [ "$x" -le "$LATCH_X1" ]; then
        if [ "$y" -ge "$LATCH_Y0" ]; then
          if [ "$y" -le "$LATCH_Y1" ]; then
            r=$LR
            g=$LG
            b=$LB
            # bevel: darker plate edge
            if [ "$x" -eq "$LATCH_X0" ]; then
              r=$(( r * 78 / 100 ))
              g=$(( g * 76 / 100 ))
              b=$(( b * 68 / 100 ))
            fi
            if [ "$x" -eq "$LATCH_X1" ]; then
              r=$(( r * 78 / 100 ))
              g=$(( g * 76 / 100 ))
              b=$(( b * 68 / 100 ))
            fi
            if [ "$y" -eq "$LATCH_Y0" ]; then
              r=$(( r * 82 / 100 ))
              g=$(( g * 80 / 100 ))
              b=$(( b * 72 / 100 ))
            fi
            if [ "$y" -eq "$LATCH_Y1" ]; then
              r=$(( r * 82 / 100 ))
              g=$(( g * 80 / 100 ))
              b=$(( b * 72 / 100 ))
            fi
            # keyhole: dark slot at the plate center (2x2)
            if [ "$x" -eq "$KEY_X" ]; then
              if [ "$y" -ge "$KEY_Y0" ]; then
                if [ "$y" -le "$KEY_Y1" ]; then
                  r=$(( r * 20 / 100 ))
