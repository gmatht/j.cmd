#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
# texture-png.sh — the PNG transparency checkerboard (light-gray/white 2×2) with a soft border
#
#   bash texture-png.sh > png.ppm      # PPM P6, 16×16 (default)
#   bash texture-png.sh --preview           # truecolor ANSI preview
#   bash texture-png.sh --png               # png-<seed>.png (needs convert)
#   TEX_SIZE=32 TEX_SEED=7 bash texture-png.sh > png32.ppm
#
...
while [ "$y" -lt "$SIZE" ]; do
  ...
  while [ "$x" -lt "$SIZE" ]; do
    ...
    # a dark 1px border so the icon reads on any block tint
    if [ "$x" -eq 0 ] || [ "$x" -eq "$LAST" ] || [ "$y" -eq 0 ] || [ "$y" -eq "$LAST" ]; then
      r=$(( r * 55 / 100 ))
      g=$(( g * 55 / 100 ))
      b=$(( b * 58 / 100 ))
    fi
...
stat_span "loop"
echo "texture-png: ${SIZE}x${SIZE}, seed $TEX_SEED" >&2
finish
