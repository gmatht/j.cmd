#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
# texture-octet.sh — raw binary data — a dark hex-dump field with pseudorandom bright bits
#
#   bash texture-octet.sh > octet.ppm      # PPM P6, 16×16 (default)
#   bash texture-octet.sh --preview           # truecolor ANSI preview
#   bash texture-octet.sh --png               # octet-<seed>.png (needs convert)
#   TEX_SIZE=32 TEX_SEED=7 bash texture-octet.sh > octet32.ppm
#
...
# the name line: the 2×3 font scaled so it spans ~90% of the width
tt_ngs=$(( ( SIZE * 9 / 10 ) / ( snmlen * 2 ) ))
if [ "$tt_ngs" -lt 2 ]; then tt_ngs=2; fi
tt_nfy0=$(( SIZE / 2 ))
tt_nsx=$(( ( SIZE - snmlen * 2 * tt_ngs ) / 2 ))
y=0
while [ "$y" -lt "$SIZE" ]; do
  x=0
  while [ "$x" -lt "$SIZE" ]; do
    ...
    if [ "$tt_on" -eq 0 ] && [ "$tt_py" -ge "$tt_nfy0" ] && [ "$tt_py" -lt $(( tt_nfy0 + 3 * tt_ngs )) ]; then
      tt_gr=$(( ( tt_py - tt_nfy0 ) / tt_ngs ))
      tt_x0=$tt_nsx
      tt_x1=$(( tt_nsx + snmlen * 2 * tt_ngs ))
      if [ "$tt_px" -ge "$tt_x0" ] && [ "$tt_px" -lt "$tt_x1" ]; then
        tt_gc=$(( ( tt_px - tt_x0 ) / ( 2 * tt_ngs ) ))
        tt_gcol=$(( ( tt_px - tt_x0 - tt_gc * 2 * tt_ngs ) / tt_ngs ))
        if [ "$tt_gcol" -lt 2 ]; then
          tt_gi=${sngi[$tt_gc]}
          tt_bitpos=$(( tt_gr * 2 + tt_gcol ))
          tt_bit=$(( ( tt_gi >> tt_bitpos ) & 1 ))
...
stat_span "loop"
echo "texture-octet: ${SIZE}x${SIZE}, seed $TEX_SEED" >&2
finish
