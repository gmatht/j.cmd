...
`;
    ...
    // diff — compare two files via the wasm-diff engine (npm package,
    // wasm vendored at www/wasm-bin/wasm-diff.wasm; a wasm-bindgen
    // library, so the command drives it directly — the glue module's
    // circular wasm import doesn't work in plain ESM). Version-gated
    // (v1 marker).
    const diffContent = `// diff v1 — compare two files (wasm-diff engine, vendored wasm)
//
// NAME
...
return 1;
`;
    this._getBackend("/bin/diff.js").read("/diff.js")
      .then((existing) => {
        if (!existing.includes("diff v1")) {
          syncWrite(this._getBackend("/bin/diff.js"), "/diff.js", diffContent);
        }
      })
...
`;
    ...
    const base64jsContent = `// base64 v1 — encode/decode base64 (RFC 4648).
...
return 0;
`;
    this._getBackend("/bin/base64.js").read("/base64.js")
      .catch(() => syncWrite(this._getBackend("/bin/base64.js"), "/base64.js", base64jsContent));
    // base32.js — base64/base32 codec command. Written only when absent.
    const base32jsContent = `// base32 v1 — encode/decode base32 (RFC 4648, A-Z2-7).
...
`;
    ...
    // llm.js — agentic coder (pi-style read/write/edit/bash core).
    // Version-gated (v2) so the agent loop reaches existing installs.
    const llmjsContent = `// llm v2 — agentic coder in the browser shell, pi-style four-tool core.
...
`;
    this._getBackend("/bin/llm.js").read("/llm.js")
      .then((existing) => {
        if (!existing.includes("llm v2")) {
          syncWrite(this._getBackend("/bin/llm.js"), "/llm.js", llmjsContent);
        }
      })
