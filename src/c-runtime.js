...
export function createCRuntime({ getMem, memory, out, err, table }) {
  ...
  const rt = {
    ...
    "$__builtin_memmove": (d, s, n) => rt["$memmove"](d, s, n),
    "$__builtin_memset": (d, c, n) => rt["$memset"](d, c, n),
    "$__builtin_memcmp": (a, b, n) => rt["$memcmp"](a, b, n),
    "$__builtin_strlen": (p) => rt["$strlen"](p),
    "$__builtin_strcpy": (d, s) => rt["$strcpy"](d, s),
    "$__builtin_strcmp": (a, b) => rt["$strcmp"](a, b),
    "$__builtin_strncpy": (d, s, n) => rt["$strncpy"](d, s, n),
    "$__builtin_strncmp": (a, b, n) => rt["$strncmp"](a, b, n),
    "$__builtin_strcat": (d, s) => rt["$strcat"](d, s),
    "$__builtin_strncat": (d, s, n) => { d = Ptr(d); s = Ptr(s); n = Ptr(n);
      const m = u8(); let i = 0; while (m[d + i]) i++;
      let j = 0; while (j < n && m[s + j]) { m[d + i + j] = m[s + j]; j++; }
      m[d + i + j] = 0; return d; },
    "$__builtin_strchr": (p, c) => rt["$strchr"](p, c),
    "$__builtin_strrchr": (p, c) => rt["$strrchr"](p, c),
    "$__builtin_strdup": (s) => rt["$strdup"](s),
    "$__builtin_free": () => {},
    "$__builtin_malloc": (n) => rt["$malloc"](n),
    "$__builtin_realloc": (p, n) => rt["$realloc"](p, n),
    "$__builtin_calloc": (n, sz) => rt["$calloc"](n, sz),
    "$__builtin_expect": (v, e) => v,
    "$__builtin_constant_p": (v) => 0,
    "$__builtin_ffs": (v) => { v = Ptr(v); if (!v) return 0; let n = 1; while (!(v & 1)) { v >>= 1; n++; } return n; },
    "$__builtin_clz": (v) => { v = Ptr(v) >>> 0; return v ? 31 - Math.floor(Math.log2(v)) : 32; },
    "$__builtin_ctz": (v) => { v = Ptr(v) >>> 0; if (!v) return 32; let n = 0; while (!(v & 1)) { v >>= 1; n++; } return n; },
    "$__builtin_popcount": (v) => { v = Ptr(v) >>> 0; let n = 0; while (v) { n += v & 1; v >>= 1; } return n; },
    "$__builtin_bswap32": (v) => { v = Ptr(v) >>> 0; return ((v & 0xff) << 24) | ((v & 0xff00) << 8) | ((v >>> 8) & 0xff00) | ((v >>> 24) & 0xff); },
