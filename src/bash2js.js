...
//
//   bash ──otranspilerl.wasm──▶ A1 shIR ──estree.js──▶ JavaScript
//
// otranspilerl.wasm is the unified debashl core + all nine backends as
// a WASI reactor (the ONE compiler binary). It parses bash and emits
// ESTree JSON; src/estree.js lowers that to JavaScript against the
// sh2.* runtime (src/sh2runtime.js) and the shell's `env` object. The
// sh2perl / sh2js / otranspiler commands are the user-facing wrappers
// over the same reactor; otranspiler is the unified interface.
//
// Usage in jtsh:
...
// ── the game's bash engine: otranspilerl (the unified A1 path) ──
// `.sh` files (runBashScript → runBash → bashToJS) run on the unified
// wasm: bash → A1 shIR → ESTree → JS, with ALL the landed perf
// transforms (native tests, inlining, typed lowering, DCE, …). The
// legacy debashcl wasm (the old parse-tree path) was removed — this is
// the only bash → JS engine.
...
// transpiled statements) and the ESTree AST. The otranspilerl A1 path
// is the only bash → JS engine (debashcl was removed).
