...
function renderFramePNG(blocks, camPos, yawDeg) {
  const W = 120, H = 90;
  const pix = new Float32Array(W * H * 3);
  const depth = new Float32Array(W * H).fill(1e9);
  // extract per-line: x y z sx sy sz r g b tx dam
  const lines = String(blocks).trim().split("\n").filter(Boolean);
  const cam = [camPos[0] + 0.5, 0.5, camPos[1] + 0.5];
  const yaw = Number(yawDeg) * Math.PI / 180;
  const c = Math.cos(yaw), s = Math.sin(yaw);
  const aPos = [-0.5,0.5,0.5, 0.5,0.5,0.5, 0.5,0.5,-0.5, -0.5,0.5,-0.5, -0.5,-0.5,0.5, 0.5,-0.5,0.5, 0.5,-0.5,-0.5, -0.5,-0.5,-0.5];
  const tris = [[0,1,2],[0,2,3],[4,5,6],[4,6,7],[0,1,5],[0,5,4],[1,2,6],[1,6,5],[2,3,7],[2,7,6],[3,0,4],[3,4,7]];
  for (const line of lines) {
    const n = line.trim().split(/[\s,]+/).filter(Boolean).map(Number);
    if (n.length < 10) continue;
    const [x, y, z, sx, sy, sz, r, g, b] = n;
    const shade = [1.0, 0.85, 0.7];
    for (const [i0, i1, i2] of tris) {
      ...
      const shadeF = shade[Math.floor(Math.random()*3)];
      for (let i = 1; i < poly.length - 1; i++) {
        const ndc = [poly[0], poly[i], poly[i+1]].map((v) => ({ x: v.w === 0 ? 1e6 : v.x / v.w, y: v.w === 0 ? 1e6 : v.y / v.w, d: v.w === 0 ? 0.5 : (v.z / v.w + 1) / 2 }));
        ...
        for (let py = sy0; py <= sy1; py++) {
          ...
          for (let px = sx0; px <= sx1; px++) {
            ...
            if (dd <= depth[idx]) {
              depth[idx] = dd;
              pix[idx*3] = r; pix[idx*3+1] = g; pix[idx*3+2] = b;
            }
          }
        }
      }
  ...
  for (let i = 0; i < W * H; i++) { body[i*3] = Math.min(255, Math.max(0, Math.round(pix[i*3]*255))); body[i*3+1] = Math.min(255, Math.max(0, Math.round(pix[i*3+1]*255))); body[i*3+2] = Math.min(255, Math.max(0, Math.round(pix[i*3+2]*255))); }
