...
// Grab the qsort builtin body: `async qsort(args) { ... },\n\n  async addr`
const m = html.match(/async qsort\(args\) \{[\s\S]*?\n  \},\n\n  async addr/);
if (!m) { console.error("FAIL: could not extract qsort builtin from index.html"); process.exit(1); }
const src = "(" + m[0].replace(/^async qsort\(args\)/, "async (args) =>").replace(/\n  \},\n\n  async addr$/, "\n}") + ")";
...
let r = await qsort(["a", "alphabetic_compare"]);
check("exit 0", r === 0);
